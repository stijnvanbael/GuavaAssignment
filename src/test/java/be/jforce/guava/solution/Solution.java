package be.jforce.guava.solution;

import be.jforce.guava.Match;
import be.jforce.guava.Team;
...
public class Solution {
    ...
    public void scored6GoalsOrMore() {
        Collection<Team> teams = Collections2.filter(TEAMS, new Predicate<Team>() {
            @Override
            public boolean apply(Team team) {
                return team.getGoals() >= 6;
            }
        });
    ...
    public void matchReportsForBelgium() {
        List<String> reports = Lists.transform(BELGIUM.getMatches(), new Function<Match, String>() {
            @Override
            public String apply(Match match) {
                return match.getTeam1().getName() + "-" + match.getTeam2().getName() + ": " + match.getGoals1() + "-" + match.getGoals2();
            }
        });
    ...
    public void indexAllQualifiedTeamsByNumberOfMatchesWonSortedByName() {
        Collection<Team> qualified = Collections2.filter(TEAMS, new Predicate<Team>() {
            @Override
            public boolean apply(Team team) {
                return team.getMatches().size() >= 4;
            }
        });
        List<Team> qualifiedSorted = Ordering.natural()
                .onResultOf(new Function<Team, Comparable>() {
                    @Override
                    public Comparable apply(Team team) {
                        return team.getName();
                    }
                }).sortedCopy(qualified);
        ListMultimap<Integer, Team> teamsByMatchesWon = Multimaps.index(qualifiedSorted, new Function<Team, Integer>() {
            @Override
            public Integer apply(Team team) {
                return team.getMatchesWon();
            }
        });
    ...
    public void sortAllQualifiedTeamsByNumberOfGoalsDescendingThenByNameAscending() {
        ...
        Ordering<Team> byGoalsReversed = Ordering.natural()
                .onResultOf(new Function<Team, Comparable>() {
                    @Override
                    public Comparable apply(Team team) {
        ...
        Ordering<Team> byName = Ordering.natural()
                .onResultOf(new Function<Team, Comparable>() {
                    @Override
                    public Comparable apply(Team team) {
                        return team.getName();
        ...
        List<Team> sortedQualified = byGoalsReversed.compound(byName).sortedCopy(qualified);
